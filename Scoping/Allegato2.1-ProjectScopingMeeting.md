# Allegato 2.1 - Project Scoping Meeting
## v.1.1.0 – 2026-08-02

## Informazioni Generali

**Data**: 15 settembre 2025
**Orario**: 14:30 - 17:00 (2h 30min)
**Luogo**: Sala Riunioni "Heritage Lab", Dipartimento di Informatica - Università di Bologna, Cesena
**Modalità**: Ibrido (3 presenti fisicamente, 4 in videoconferenza)

---

## Partecipanti

### PlayHeritage Labs (Team di Sviluppo)

| Nome | Ruolo | Presenza |
|------|-------|----------|
| **Dr. Marco Venturi** | CEO & Project Manager | Presenza fisica |
| **Elena Rossi** | Lead Developer & Game Engine Specialist | Presenza fisica |
| **Luca Moretti** | UX/UI Designer & Cultural Heritage Expert | Presenza fisica |
| **Sara Bianchi** | Backend Developer | Videoconferenza |
| **Andrea Conti** | DevOps & Infrastructure Specialist | Videoconferenza |

### Maraffa Forever (Committente)

| Nome | Ruolo | Presenza |
|------|-------|----------|
| **Giovanni Marchetti** | Project Sponsor & Rappresentante Community | Videoconferenza |
| **Francesca Giuliani** | Esperta di Maraffa & Beta Tester Lead | Videoconferenza |

---

## Obiettivi del Meeting

1. **Presentazione formale del progetto** da parte di PlayHeritage Labs
2. **Comprensione delle esigenze** della community "Maraffa Forever"
3. **Definizione degli obiettivi di alto livello** del progetto
4. **Allineamento sulle aspettative** di budget, tempi e qualità
5. **Identificazione preliminare dei rischi** principali
6. **Definizione delle modalità di comunicazione** tra team e committente

---

## Agenda del Meeting

### 1. Apertura e Presentazioni (14:30 - 14:45)
- Presentazione dei partecipanti
- Panoramica sull'expertise di PlayHeritage Labs
- Storia della community "Maraffa Forever"

### 2. Presentazione del Problema e Opportunità (14:45 - 15:15)
- **Giovanni Marchetti** presenta la situazione attuale della community
- Mancanza di piattaforme digitali di qualità per giocare a Maraffa online
- Dispersione geografica dei membri (Italia ed Europa)
- Nostalgia per l'esperienza sociale del gioco dal vivo

### 3. Visione del Progetto MaraffaOnline (15:15 - 15:45)
- **Marco Venturi** presenta la visione di PlayHeritage Labs
- Obiettivo: piattaforma web e mobile per giocare a Maraffa in multiplayer
- Timeline prevista: 7 mesi (15/10/2025 - 15/05/2026)
- Budget disponibile: €25.000

**Pausa caffè (15:45 - 16:00)**

### 4. Requirements di Alto Livello (16:00 - 16:30)
- **Francesca Giuliani** specifica le regole della Maraffa tradizionale romagnola
- Discussione sulle funzionalità must-have vs nice-to-have:
  - Partite multiplayer in tempo reale (4 giocatori)
  - Stanze private per giocare con amici specifici
  - Chat in-game
  - Interfaccia intuitiva e moderna
  - [Nice to have] App mobile nativa (da valutare vs responsive web)
  - [Nice to have] Sistema di classifiche e statistiche
  - [Nice to have] Modalità torneo

### 5. Rischi e Vincoli (16:30 - 16:45)
Identificazione preliminare dei rischi principali:
- **Rischio tecnico alto**: sincronizzazione real-time tra 4 giocatori
- **Rischio di budget**: €25.000 è un budget limitato per 7 mesi di sviluppo
- **Rischio di scope creep**: necessità di prioritizzare le features
- **Vincolo temporale**: la community desidera lanciare la piattaforma entro maggio 2026

### 6. Modalità di Lavoro e Comunicazione (16:45 - 16:55)
Accordi su:
- **Meeting bi-settimanali** di avanzamento (ogni martedì alle 18:00 via Zoom)
- **Canale Slack dedicato** per comunicazioni rapide
- **Demo mensili** per mostrare progressi al committente
- **Beta testing** con 20 membri selezionati della community

### 7. Prossimi Passi e Chiusura (16:55 - 17:00)
- Approvazione formale dell'inizio del progetto
- Definizione del contratto formale (milestone e pagamenti scaglionati) entro fine settembre
- Firma del contratto e kick-off ufficiale del progetto: 15/10/2025, a valle dell'approvazione dello Scoping e del Planning

---

## Decisioni Prese

| # | Decisione | Responsabile | Scadenza |
|---|-----------|--------------|----------|
| 1 | Approva il budget di €25.000 e la durata di 7 mesi | Giovanni Marchetti | Approvato |
| 2 | Prioritizzare versione web responsive prima dell'app mobile nativa | Marco Venturi | Approvato |
| 3 | Definire contratto formale con milestones e pagamenti scaglionati | Marco Venturi | 20/09/2025 |
| 4 | Identificare 20 beta tester dalla community per user testing | Francesca Giuliani | 30/09/2025 |
| 5 | Creare canale Slack condiviso PlayHeritage-MaraffaForever | Andrea Conti | 17/09/2025 |
| 6 | Programmare primo meeting bi-settimanale | Marco Venturi | 26/09/2025 |
| 7 | Preparare documentazione dettagliata delle regole della Maraffa | Francesca Giuliani | 25/09/2025 |

---

## Action Items

| # | Azione | Assegnato a | Scadenza | Stato |
|---|--------|-------------|----------|-------|
| 1 | Redigere il Project Overview Statement (POS) | Marco Venturi | 22/09/2025 | Not started |
| 2 | Preparare documento Conditions of Satisfaction | Marco Venturi | 22/09/2025 | Not started |
| 3 | Condurre Risk Analysis preliminare | Elena Rossi | 23/09/2025 | Not started |
| 4 | Creare primi wireframe/mockup dell'interfaccia | Luca Moretti | 28/09/2025 | Not started |
| 5 | Inviare regolamento ufficiale Maraffa romagnola | Francesca Giuliani | 25/09/2025 | Not started |
| 6 | Identificare e contattare 20 beta tester | Francesca Giuliani | 30/09/2025 | Not started |
| 7 | Preparare bozza di contratto con milestone payments | Marco Venturi | 19/09/2025 | Not started |
| 8 | Setup ambiente Slack e invito partecipanti | Andrea Conti | 17/09/2025 | Not started |

---

## Note e Osservazioni

### Aspettative del Committente
- **Giovanni Marchetti** ha sottolineato l'importanza della fedeltà alle regole tradizionali: "Non vogliamo un gioco inventato, ma la Maraffa vera, quella che giocavamo all'osteria"
- La community è disposta a partecipare attivamente ai test e fornire feedback continuo
- Preferenza per un'interfaccia "calda" e conviviale, che ricrei l'atmosfera del tavolo da gioco reale

### Opportunità Identificate
- **Francesca Giuliani** ha menzionato che altre community di giocatori di carte regionali (Briscola chiamata, Tresette) potrebbero essere interessate a soluzioni simili
- Possibilità di pubblicazione scientifica per PlayHeritage Labs sul tema "cultural heritage gaming"
- Il progetto sarà utilizzato come caso di studio per la tesi di dottorato di Marco Venturi

### Preoccupazioni Emerse
- Budget limitato (€25.000) per un progetto di 7 mesi con 5 persone nel team
- Incertezza tecnica sulla gestione del real-time multiplayer (esperienza limitata del team)
- Necessità di bilanciare qualità e velocità di sviluppo

---

## Allegati

- Registrazione audio del meeting (disponibile su richiesta)

---

**Redatto da**: Marco Venturi
**Approvato da**: Giovanni Marchetti
**Data approvazione**: 16/09/2025

**Storico revisioni**:
- **v.1.1.0**: Corrette le date previste per contratto e kick-off (erano 20/09 e 25/09, incompatibili con la chiusura dello Scoping del 02/10): la firma del contratto e il kick-off sono il 15/10/2025, come da Allegato 4.1 e Cash Flow.
- **v.1.0.0**: Prima stesura del verbale.
