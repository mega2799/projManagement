...
23 storie in 7 epiche (un'epica è una storia troppo grande, da spezzare), nel formato "Come <ruolo> voglio <azione> così da <beneficio>", ognuna con criteri di accettazione puntuali e approccio di test. INVEST (Wake) è la checklist di qualità di una storia: **I**ndependent (indipendente dalle altre), **N**egotiable (negoziabile nel come), **V**aluable (porta valore all'utente), **E**stimable (stimabile), **S**mall (piccola), **T**estable (verificabile) — verificata in tabella criterio per criterio. Scelta di pulizia: nell'allegato delle User Stories (2.8) non ci sono priorità, story point né sprint, perché appartengono al Planning (Allegati 3.2 e 3.3): separazione tra il *cosa* (Scoping) e il *quanto/quando* (Planning).

**B11. Differenza RBS/WBS?**
La **RBS** (Requirements Breakdown Structure) è la gerarchia dei **requisiti** (Requirement → Function → Sub-function → Feature): dice *cosa* va consegnato, con un linguaggio comprensibile al committente. La **WBS** (Work Breakdown Structure) è la gerarchia del **lavoro** necessario a soddisfarli: dice *come si organizza il fare*. Nel processo di Wysocki la completezza della RBS è il fattore principale per scegliere il ciclo di vita: da noi, RBS del Game Engine completa e stabile → Waterfall; RBS del Real-Time incerta → Adattivo. **Livello di dettaglio** (scelta deliberata, dichiarata nell'allegato): la RBS si ferma al requisito — dice *cosa*, non *come*. I criteri di accettazione stanno nelle User Stories, le soglie misurabili nelle CoS, le scelte tecnologiche nel Planning; la tracciabilità è garantita dagli ID `REQ-*` della MoSCoW, che codificano la numerazione della RBS.
...
I sei passi canonici, tutti eseguiti: (1) **accettazione formale** del deliverable → firma di Giovanni lunedì 11/05, dopo aver **giocato personalmente una partita completa** (accettazione esperienziale, non solo documentale); (2) **installazione** → il deploy in produzione è a carico della community, come dichiarato fin dall'inizio: consegnate le immagini Docker con le istruzioni e il supporto remoto; go-live con **approccio a fasi** (beta dei 20 tester → rilascio pubblico), che era anche la mitigazione del rischio "picchi di traffico" — cut-over e parallel non applicabili, non c'era una soluzione preesistente; (3) **documentazione** completa → il **Project Notebook** su Notion, alimentato fin dal primo giorno (POS, RBS e revisioni, schedule, verbali, status report, change request), + allegati; (4) **Final Project Report** firmato → consegnato il 12/05, sintesi del Notebook strutturata sul template del corso (Executive Summary, livello di successo, organizzazione, tecniche, pregi e difetti, raccomandazioni, appendici); (5) **post-implementation audit** → il 13/05, con lo sponsor, criterio per criterio sulle Conditions of Satisfaction; (6) **celebrazione** → il 15/05, con una voce di budget dedicata (€100): persino la festa è pianificata, come vogliono le slide.
...
Sì, due refusi dell'Allegato 2.8, individuati con un audit interno e **corretti** (insieme al punteggio d'esempio "41-38", impossibile col sistema a 11 punti interi per smazzata, ora "41-36"): la fonte normativa — il documento delle regole validato dall'esperta, richiamato dalla RBS come riferimento per il Game Engine — ha sempre detto correttamente che inizia e sceglie la briscola **chi ha il 4 di Denari**, e che la Maraffa è **Asso+2+3 del seme di briscola**. La catena di sicurezza del progetto è proprio questa: la specifica validata dall'esperta è la fonte di verità, e infatti in beta risultano zero segnalazioni di errori sulle regole.
...
**G15. Perché lo Scoping Meeting e l'Approval Process non hanno un allegato? E perché il numero di allegato non coincide con la sezione della relazione?**
Scelta di asciugatura calibrata sulla relazione di riferimento: i **verbali interni** non sono artefatti di Project Management — il loro contenuto informativo (partecipanti, decisioni, riserve, esito) sta per intero in poche righe, e la relazione infatti li narra direttamente nelle sezioni 2.1 e 2.11. Gli allegati restano riservati agli **artefatti veri e propri** (CoS, POS, Risk Matrix, BMC, SWOT, Prototyping, RBS, US, PMLC) e sono numerati **sequenzialmente 2.1–2.9**: per questo il numero di allegato non coincide con il numero di sezione. All'orale: citare i due meeting come *eventi*, con data ed esito (Scoping Meeting 15/09; approvazione formale 02/10, due giorni oltre la data prevista, entro il margine).

**G16. Le vostre tabelle CPM dicono EF = ES + durata, ma le slide insegnano EF = ES + durata − 1: quale è giusta?**
Entrambe: sono due convenzioni equivalenti, e l'Allegato 3.5 lo dichiara. Le slide contano *giorni di calendario* 1-based (il task di durata 1 inizia e finisce il giorno 1: EF = ES + durata − 1, LS = LF − durata + 1); le nostre tabelle usano *istanti cumulati* 0-based (il valore è il confine di fine giornata: ES=0, EF=9 significa "occupa i primi 9 giorni", quindi EF = ES + durata). Le due notazioni differiscono di una costante: durate, slack e critical path sono identici. Se all'orale si ricalcola alla lavagna con la formula delle slide, basta traslare tutto di 1.
...
| FS / SS / FF / SF + lag | i 4 tipi di dipendenza (Finish-to-Start la classica; Start-to-Start = la successiva non può iniziare prima che *inizi* la precedente); lag = ritardo imposto sul legame |
