...
## 2. La scelta regina: approccio ibrido per sottosistema (Allegato 2.9)
...
## 3. Scelte di Scoping (relazione §2.1–2.11 · Allegati 2.1–2.9)
...
- Meeting del 15/09/2025 con **tutti** gli attori (sponsor Giovanni Marchetti, esperta di dominio Francesca Giuliani, team completo), in modalità ibrida. Decisioni chiave: approccio **web-first** (l'app nativa è rinviata), contratto a milestone, 20 beta tester dalla community, regole del gioco documentate dall'esperta.
- **Scelta di consegna (2026-08-23)**: il verbale **non è più un allegato** — il meeting è narrato nella sezione 2.1 della relazione, senza documento a parte (il file resta nel repo come nota interna, `_ritirato-ProjectScopingMeeting.md`); idem per l'Approval Process (ex 2.11). Gli allegati di Scoping sono stati **rinumerati sequenzialmente 2.1–2.9** come nella relazione di riferimento: attenzione all'orale, il numero di allegato non coincide più con la sezione della relazione ("Allegato 2.1" ora è la **CoS**).
...
- **Scelta di consegna (2026-08-23)**: come per lo Scoping Meeting, il verbale **non è più un allegato** — l'esito è narrato nella sezione 2.11 della relazione (stile della relazione di riferimento, dove l'Approval Process è una sezioncina con partecipanti ed esito). Il file resta nel repo come nota interna (`_ritirato-ScopingApprovalProcess.md`).
...
- **Doppia lettura dichiarata** (v.1.3.0): i livelli 1–3 — le **43 attività**, work package da ≈7 giorni-uomo dentro la *8/80 rule* — sono la WBS **gestionale**, su cui si esercitano stime, RASCI e schedulazione (il network lavora su 20 attività aggregate, raccordate dalla **tabella di tracciabilità dell'Allegato 3.5**); il 4° livello è la **scomposizione operativa** che alimenta lo Sprint Backlog. Risponde alla domanda "non è troppo dettagliata?": il dettaglio è *thought process tool*, il controllo sta al livello giusto.
...
>
> Le **dipendenze** possono essere: **FS** (Finish-to-Start, la classica: B inizia quando A finisce), **SS** (Start-to-Start: B non può iniziare prima che A *inizi*), FF, SF; il **lag** è un ritardo imposto sul legame (SS+31 = B inizia 31 giorni dopo l'inizio di A).
>
...
- **Raccordo con la WBS documentato** (v.2.1.0): una tabella di tracciabilità mappa ciascuno dei 20 nodi di rete sulle attività della WBS che aggrega, con nota esplicita sulle attività continuative/on-demand che non formano nodi (documentazione PM, incrementi successivi di monitoring e sicurezza, chat globale Could). Chiude la domanda "come si passa dai 160 task alle 20 attività del Gantt?".
...
## 5. Scelte di Launching (Allegati 4.1–4.3)
...
- **Granularità a livello di attività** (v.2.1.0): 51 righe — i task ereditano la riga dell'attività della WBS a cui appartengono, le attività con distribuzione di ruoli diversa hanno riga dedicata. La versione per-task (124 righe) ripeteva lo stesso pattern di ruoli nell'82% delle righe, seppellendo le eccezioni che la RASCI deve far risaltare; nessuna assegnazione è cambiata nella potatura.
...
- **Final Project Report (12/05)** come **sintesi del Project Notebook** (il workspace Notion alimentato dal primo giorno), strutturato sul template del corso (Executive Summary → livello di successo → organizzazione → tecniche → pregi e difetti → raccomandazioni → appendici); **audit post-implementazione (13/05) condotto insieme allo sponsor**, verificando ogni criterio delle CoS — qui paga la tracciabilità numerica del POS: 7 mesi rispettati, €22.750 su €25.000, latenza media 185ms, 80% di utenti autonomi alla prima partita, beta tester 4,5/5 contro il target 4,2.
...
1. **US-3.1 (Allegato 2.8): "Determinazione casuale primo giocatore"** — contraddiceva le regole ufficiali richiamate dalla RBS 1.1.2 (inizia e sceglie la briscola chi ha il **4 di Denari**). Errore di fedeltà alle regole, cioè sull'obiettivo n. 1 del POS. **Corretto.**
2. **US-3.6 (Allegato 2.8): Maraffa descritta come "3 carte dello stesso seme"** — la Maraffa/Cricca è **Asso+2+3 del seme di briscola** (il documento delle regole, riferimento normativo della RBS 1.1.5, era corretto). **Corretto** (inclusi i criteri di accettazione e l'obbligo di giocare l'Asso per primo).
...
**Secondo giro di audit (2026-08-09, 5 squadre: date/calendario, denaro, cast/riferimenti, regole di gioco, metriche) — esito.** Trovati e **corretti**: la griglia sprint del Backlog che correva da martedì a sabato (riallineata alla griglia lun–ven del Gantt: S1 28 Ott–7 Nov … S5 22 Dic–2 Gen … S11 16–31 Mar; etichette del CSV riallineate); l'accettazione formale datata domenica 10/05 → **lunedì 11/05** (Cap. 6 + relazione); il punteggio impossibile "41-38" in US-3.5 → "41-36" (79 non è esprimibile come 11n+3m: ogni smazzata distribuisce 11 punti interi più eventuali bonus Maraffa da 3); il totale punti per smazzata uniformato (10⅔ dalle carte + 1 di ultima presa = 11⅔ grezzi → 11 interi dopo l'arrotondamento per difetto, in RBS e RASCI; dopo l'asciugatura della RBS v.1.1.0 il dettaglio numerico del punteggio vive nel documento delle regole e nelle US); il lessico presa/smazzata dove il troncamento morde; il criterio busso/striscio/volo aggiunto a US-3.2; il break-even ottimistico ≈50 → **≈84 mesi** (al netto dei costi operativi); il PoC raccontato in due stadi coerenti (spike in Sprint 0 → PoC completo in Sprint 2); le milestone del network rinumerate M2/M3/M4/M6 (erano M1–M4 in conflitto con la tabella M1–M7, anche nello script HTML); il ritardo dello Sprint 7 attribuito all'attività M anche in §5.8; i codici rischio 2.1/6.2 al posto degli inesistenti R1/R4; tornei = Won't (non Could) nel Closing; il tutorial "Scopri le regole" non più spacciato per extra (era pianificato allo Sprint 6); PWA = Won't ovunque; gli SP dei task padre del CSV = somma dei figli (9 righe); notifiche in-app P1/Must e sidebar amici P2 nel Backlog/CSV; la sensitivity di N/Q precisata (i task Must di Q non sono de-scopabili); REQ-SOC-6.1 riqualificato come quota UI delle amicizie (niente doppio conteggio); "12 allegati" → 11; la chat 1-a-1 fantasma → chat globale lobby; PaaS/DigitalOcean/Confluence → stack canonico; coverage riconciliata (80% gate di merge, 85% target di progetto); riga chat in-game aggiunta alla RASCI; reconnection "30s" riformulata (heartbeat 30s, sospensione 5 min); le date nel weekend spostate a giorni feriali (approvazioni 2.4/2.9, materiali del kick-off venerdì 10/10, primo Status Meeting venerdì 17/10, Sprint 0 Review venerdì 24/10, review 19/12); l'action item del kick-off "approvazione Scoping/Planning" riqualificato in approvazione del verbale; la nota di superamento degli accordi preliminari nel 2.1; W1/W4/W3 della SWOT allineate; le versioni dei companion HTML allineate ai .md.

**Terzo giro di audit (2026-08-23, teoria contro knowledge_base, 6 lenti: Scoping, Planning, Launching, Monitoring, Closing, PMLC/Agile) — esito.** Corretti: la definizione MoSCoW degli **Should Have** ("importanti ma non critici" → "critici, sostituibili se necessario", come da slide — anche nel Cap. 3 della relazione); la voce di legenda **SS** del 3.5 (B e C dopo A partono insieme per *divergenza* di legami FS, non per vincolo Start-to-Start) e dichiarata la **convenzione dei tempi** 0-based a istanti vs 1-based a giorni delle slide (risposta pronta in FAQ G16); la **doppia lettura WBS** riformulata per non contraddire "task = livello di stima" del corso (quel ruolo lo svolgono le 43 attività); il rischio 1.2 da **Accept ad Avoid** (adottare la versione ufficiale elimina la fonte del rischio; per il corso Accept = nessuna azione possibile) e il 4.2 da **Contingenza a Mitigazione** (azioni tutte preventive immediate); la tabella PMLC del 2.9 riallineata al **quadrante di Wysocki** (l'Iterativo diceva "requisiti noti", che è la condizione del TPM; via "Scrum-like" dall'Iterativo e "Kanban" come nome dell'Adattivo — per le slide Scrum è Adaptive e il PMLC di Kanban è iterativo, la board a flusso resta come strumento; Incrementale = TPM a requisiti definiti); sei righe RASCI dell'area PM con **A senza R** → **A/R** su Marco; introdotto il **Project Notebook** come contenitore della documentazione (workspace Notion dal primo giorno) con il **FPR come sintesi ristrutturata sulle 7 voci del template del corso**; la domanda dell'audit sul business value a regime affidata esplicitamente al monitoraggio post-lancio; la colonna "Net Cash Flow" rinominata secondo le slide (si calcola sui totali cumulati); il residuo "US (2.9)" → "US (2.8)" nel Backlog. **Verificati senza rilievi**: formule e segni EVM (CV/SV/CPI/SPI), stoplight, scala di escalation, issue log, POS, ciclo CoS, kick-off, regole operative (stili decisionali, meeting), change management, INVEST, Delphi/Planning Poker, definizioni di progetto/PM.
