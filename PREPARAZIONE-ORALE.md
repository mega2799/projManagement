...
| WBS | 4 livelli, 100% rule; **doppia lettura**: 43 attività di controllo (≈7 gg-uomo, 8/80 rule) + 160 task operativi (→ Sprint Backlog) |
...
### Scoping (relazione §2.1–2.11 · Allegati 2.1–2.9)
...
La **Risk Analysis** (2.4) usa la Risk Rating Matrix probabilità×impatto: 20 rischi, di cui **2 critici a rating 16** — l'inesperienza del team con WebSocket (contingenza: spike tecnico di 2 settimane + consulente Nardi a €2.000) e il budget risicato (accettato: team part-time, valore strategico) — più scope creep (12, mitigato con Change Control rigoroso e freeze dei requisiti dopo il POS) e race conditions (12, architettura server-authoritative). **BMC** (2.5) e **SWOT** (2.6) inquadrano la strategia: punto di forza la co-creazione con una community motivata (20 beta tester), debolezza chiave la stessa inesperienza real-time. Il **Prototyping** (2.7) produce mockup Figma in due versioni — v1 commentata da Giovanni, v2 approvata e **vincolante** per il frontend. La **RBS** (2.8) organizza i requisiti per sottosistema, le **User Stories** (2.9) li esprimono secondo **INVEST**, e il **Life Cycle Models** (2.10) assegna la metodologia a ogni sottosistema (vedi §6.1). Chiusura con **Approval Process** (2.11): approvazione formale del perimetro il 2/10, narrata in relazione senza allegato — sponsor unico voto decisionale, due riserve (monitoraggio WebSocket; sistema amicizie Should condizionato ai tempi), due giorni di scostamento entro il margine. *(Nota: i numeri tra parentesi qui sono le sezioni della relazione; gli allegati sono numerati sequenzialmente 2.1–2.9 — vedi la mappa in §8.)*

#### Come si arriva alla Risk Analysis e alla matrice (percorso)

I rischi non si inventano: escono da documenti già scritti (POS, SWOT, RBS, CoS) e si spazzolano contro le **4 categorie del corso** — Technical, Project Management, Organizational, External. Ogni rischio è codificato `sottosistema.rischio` (1.x Game Engine, 2.x Real-Time, 3.x Frontend, 4.x Backend/Social, 5.x Infrastructure, 6.x PM), così la matrice dice anche *quale pezzo del sistema* è pericoloso.

```
POS + SWOT + RBS + CoS
   ↓  identificazione (4 categorie del corso, codifica sottosistema.rischio)
20 rischi
   ↓  valutazione (probabilità A-D × impatto 1-4)
valore 1-16
   ↓  posizionamento
RISK RATING MATRIX (4×4, colori = livelli di azione)
   ↓  per ciascuno
strategia (Accept / Avoid / Mitigate / Contingency / Transfer)
   ↓  durante il progetto
Risk Log aggiornato → 2.1 scende da 16 a 8 → chiuso come mitigato
```

**Valore di rischio = Probabilità × Impatto** (1-16): è l'**impatto atteso** del corso in versione *qualitativa*. La versione quantitativa (l'esempio delle slide: 1% × €50.000 = €500) è stata scartata perché per rischi come "il team non sa fare WebSocket" non esiste una cifra credibile: darla sarebbe **falsa precisione**. Il colore **determina l'azione** (rosso critico → contingenza dedicata): la decisione la prende la regola, non l'umore del momento. Distribuzione: **2 rosso critico · 6 rossi · 7 arancioni · 5 gialli · 0 verdi** — nessun verde perché nel registro sono entrati solo i rischi che meritavano di esserci, non per gonfiare il volume.

**Le 5 strategie di risposta del corso** (Allegato 2.3):

| Strategia | In parole | Il nostro caso |
|---|---|---|
| **Accept** | non c'è niente da fare, lo si accetta consapevolmente | **6.1** budget insufficiente: il crowdfunding è quello, nessuno lo assicura — accettato in sede di approvazione |
| **Avoid** | si cambia il piano per far sparire la situazione | **1.2** varianti regionali: adottata la versione ufficiale "Maraffa Forever", l'ambiguità non esiste più |
| **Mitigate** | si agisce **subito** per ridurre probabilità o impatto | **2.4** race conditions: architettura server-authoritative + code review obbligatoria |
| **Contingency** | si prepara **ora** un piano da eseguire **se** l'evento accade | **2.1** WebSocket: consulente identificato, €2.000 accantonati, go/no-go al giorno 15, escalation al giorno 30 |
| **Transfer** | si sposta l'impatto su altri (assicurazione, outsourcing) | non usata: nessun rischio era assicurabile o esternalizzabile |

**La distinzione che cade sempre**: *Mitigate* è **adesso**, *Contingency* è **se**. Sul WebSocket non si poteva mitigare un'incognita di fattibilità (o funziona o no): si è ridotta l'incertezza con lo spike e si è preparato il piano B con una data di attivazione.

**Assessment dinamico, non statico** (il corso lo indica come approccio migliore): il rischio **2.1 è cambiato nel tempo** — il PoC dello Sprint 2 misura 180 ms di latenza e il rating scende **da 16 a 8**; il load test dello Sprint 6 lo chiude come "Mitigato". Il registro è vivo, non un documento di settembre.

**Tre trappole**: (1) rischio ≠ problema — il rischio è futuro e incerto, quando si verifica diventa un *issue* e passa all'issue log; (2) rating alto ≠ azione costosa — il 6.1 ha rating 16 ed è *Accept*, perché non era né mitigabile né trasferibile; (3) SWOT ≠ Risk Analysis — la SWOT classifica **condizioni**, la Risk Analysis valuta **eventi** con probabilità e impatto (una condizione genera un rischio).

### Planning (Allegati 3.1–3.5)

La **WBS** (3.1) decompone il lavoro su 4 livelli (100% rule) per i 6 sottosistemi attivi + attività trasversali di PM/QA, con **doppia lettura dichiarata**: il controllo si esercita sulle 43 attività (work package da ≈7 gg-uomo, dentro la 8/80 rule; il network lavora su 20 attività aggregate, raccordate dalla tabella di tracciabilità del 3.5), mentre i 160 task del 4° livello sono la scomposizione operativa che alimenta lo Sprint Backlog. La **MoSCoW** (3.2) prioritizza: 302 giorni-uomo con Must al 75,8% — sopra il target DSDM del 60%, scelta motivata dalla criticità di Game Engine e Real-Time — e Could compressi al 5,3% in funzione anti-scope-creep. Le **stime** usano **Delphi** per il Game Engine (Waterfall: stime anonime, più round fino a convergenza) e **Planning Poker** per i sottosistemi Agile (Fibonacci 1–21). Il **Product Backlog** (3.3) raccoglie il lavoro che scorre negli sprint (310 SP, vedi §6.2). Il **Cash Flow** (3.4) mostra saldo cumulativo sempre positivo con minimo €2.250 nell'ultimo mese. Il **Network Diagram + Gantt** (3.5) applica il CPM: percorso critico di 141 giorni lavorativi con fast tracking dichiarato (vedi §6.3). La fase si chiude con il meeting di approvazione della pianificazione.

### Launching (Allegati 4.1–4.3)

**Kick-Off Meeting** il 15 Ott 2025 con team, sponsor ed esperta: visione, perimetro MVP, timeline con milestone, budget, rischi, regole operative. La **RASCI** (4.2) assegna Responsible/Accountable/Support/Consulted/Informed **a livello di attività della WBS** (51 righe: i task ereditano la riga dell'attività, le eccezioni — validazione regole, UAT, badge, disaster recovery — hanno riga dedicata), con la regola **un solo Accountable per attività**. Le **regole operative** comprendono: problem solving con root cause analysis (**5 Whys**), decision making su **3 livelli** (operativo/tattico/strategico, "decidere al livello più basso possibile"), conflict resolution progressiva in **3 fasi** (discussione diretta → mediazione facilitata → decisione esecutiva; mai servita oltre la fase 2), brainstorming e cerimonie (Daily Standup 9:00–9:15, Sprint Planning, Backlog Refinement a metà sprint, Review, Retrospective, Project Status Meeting). **Comunicazione**: asincrona-first su Slack/email, Notion come *single source of truth*, SLA di risposta per priorità. **Gestione del cambiamento**: Change Request formalizzato (submission → impact analysis con Project Impact Statement → decisione sponsor → implementazione → tracciamento); i sottosistemi Agile accolgono il cambiamento nel processo, quelli tradizionali lo valutano caso per caso.
...
I criteri di scelta sono **l'incertezza dei requisiti e l'incertezza della soluzione** (matrice in Allegato 2.9):
...
No, e la chiave è distinguere **capacità** da **carico**. I 40 SP/sprint sono la **capacity team-wide**: quanto il team *può* completare in uno sprint, includendo il lavoro di Game Engine e Infrastructure stimato in punti equivalenti (≈131 SP, per un totale progetto di ≈441). Il **carico medio pianificato** è più basso, e sono due conti da saper rifare a voce: `441 ÷ 12 sprint (0-11) = 36,75` ≈ **37 SP/sprint team-wide** e `310 ÷ 11 sprint (1-11) = 28,2` ≈ **28 SP/sprint di solo Backlog** — i denominatori differiscono perché lo Sprint 0 è di setup (contiene lavoro, quindi pesa nei 441; nessuna user story, quindi non pesa nei 310). Il margine tra capacità e carico assorbe festività (Sprint 5 natalizio, 21/40 SP con 19 di carryover) e variabilità delle stime. Gli Sprint 12-14 non sono stimati in SP (UAT, lancio); lo Sprint 11 ospita solo le rifiniture del tavolo, in fast tracking col testing E2E. Due livelli di lettura: metrica *gestionale* consolidata per lo sponsor (quella monitorata nel Cap. 5) e carico *Scrum* del team. La velocity misurata media è **38,3**, coerente con la capacity 40.
...
### 6.10 «La WBS non è troppo dettagliata? "Definire struttura dati per carte" non è implementazione?» ⚠️

La WBS ha una **doppia lettura dichiarata** (Allegato 3.1 v.1.3.0). Il livello di **controllo gestionale** sono le **43 attività**: work package da ≈7 giorni-uomo in media, dentro la finestra della **8/80 rule** (8–80 ore), su cui si esercitano stime (MoSCoW), responsabilità (RASCI) e schedulazione — il Project Network Diagram lavora infatti su 20 attività aggregate, raccordate alla WBS dalla **tabella di tracciabilità dell'Allegato 3.5**. I **160 task** del 4° livello sono la **scomposizione operativa** (WBS Dictionary): alimentano lo Sprint Backlog dei sottosistemi agili e garantiscono la tracciabilità requisito → lavoro; non sono l'unità di stima né di controllo (un task medio vale ≈2 giorni-uomo). Il dettaglio "tecnico" dei titoli non è implementazione ma **thought process tool** (uno dei 4 usi della WBS secondo il corso): dimostra la padronanza del dominio necessaria a stimare. Coerentemente, la **RASCI è a livello di attività** (51 righe): una matrice per task ripeterebbe 124 volte pochi pattern di ruoli, seppellendo le eccezioni che deve far risaltare.

### 6.11 Definizioni-lampo che possono chiedere sul progetto
...
- **RASCI**: un solo Accountable per attività; Responsible esegue; matrice a livello di attività WBS (i task ereditano la riga).
...
| `FAQ.md` | 62 domande con risposta (A–G) + **17 di teoria applicata al progetto (H)** + definizioni-lampo (I) |
| `SCELTE-DI-PROGETTO.md` | il perché di ogni scelta, con riquadri di ripasso teorico e registro degli audit (§8) |
| `DRILL-ORALE.md` | ≈100 domande-lampo con risposta in una riga, per il ripasso attivo degli ultimi giorni |
| `../studio/domande_del_professore.md` | le 28 domande reali del prof dalle dispense, ognuna ora con un **Aggancio a MaraffaOnline** |
| `../studio/simulazione_esame.md`, `guida_elaborato.md` | simulazione d'esame in 3 parti; guida fase→deliverable (riallineata alla numerazione attuale) |
| Scoping Meeting | solo narrato nella relazione (§2.1), senza allegato — verbale ritirato il 2026-08-23 (decisioni: web-first, contratto a milestone) |
| 2.1 CoS | 5 tipologie di criteri negoziati richiesta/risposta |
| 2.2 POS | problema, opportunità, goal, 6 obiettivi, criteri, rischi, assunzioni, ostacoli |
| 2.3 Risk Analysis (+HTML) | matrice 20 rischi, 2 critici, mitigazioni |
| 2.4 BMC (+HTML) / 2.5 SWOT (+HTML) | inquadramento strategico |
| 2.6 Prototyping | mockup v1 commentata → v2 approvata (vincolante) |
| 2.7 RBS / 2.8 User Stories | requisiti per sottosistema, fermi al livello di requisito (dettaglio nelle US/CoS) / INVEST. Metodo di raccolta (5 tecniche su 7) e valutazione di completezza: non scritti nell'allegato, risposta pronta in FAQ B14 |
| 2.9 Life Cycle Models | matrice metodologia×sottosistema con motivazioni |
| Approval Process | solo narrato nella relazione (§2.11), senza allegato — verbale ritirato come lo Scoping Meeting |
| 3.1 WBS (+visuale) | 4 livelli; 43 attività di controllo + 160 task operativi (doppia lettura) |
...
| 4.2 RASCI Matrix (+visuale) | 51 righe a livello attività (8 aree; i task ereditano la riga), un solo Accountable per attività |
| 4.3 Regole Operative | 5 Whys, 3 livelli decisionali, conflict resolution in 3 fasi, brainstorming, cerimonie (comunicazione e change management sono solo prosa in relazione, §4.4-4.5, senza rimando all'allegato) |
...
**Ultimo aggiornamento**: 2026-09-09 — **§5 Scoping: aggiunto il blocco "Come si arriva alla Risk Analysis e alla matrice"** — il percorso dai documenti di input (POS/SWOT/RBS/CoS) alle 4 categorie del corso, la codifica `sottosistema.rischio`, il diagramma del flusso fino al Risk Log, la formula `P × I` con il motivo della versione qualitativa e non in euro (falsa precisione), la distribuzione dei 20 rischi con la nota sull'assenza di verdi, la **tabella delle 5 strategie di risposta** (Accept/Avoid/Mitigate/Contingency/Transfer) con il caso corrispondente del progetto, la distinzione Mitigate-adesso vs Contingency-se, l'assessment dinamico dimostrato dal rischio 2.1 (16 → 8 → mitigato) e tre trappole (rischio ≠ problema; rating alto ≠ azione costosa; SWOT classifica condizioni, la Risk Analysis valuta eventi). Documenti di consegna non toccati. Aggiornamento precedente: 2026-09-08 — nuovo **`RIPASSO-SCELTE.md`**: foglio di sintesi da due pagine per l'ultimo ripasso (due registri, quattro ancore dei numeri, cash flow in 30 secondi, scelte-cardine col costo dell'alternativa, punti deboli da anticipare, sei "cosa farei di diverso", frasi pronte) — estratto dalle fonti, nessun contenuto nuovo. In giornata: **`SCELTE-DI-PROGETTO.md` revisionato sul formato dell'orale** (il docente ripercorre le scelte e chiede come sono state prese, perché non altre, cosa cambieresti): nuove **§1.5** (rispondere alle domande di processo: i due registri narrazione/progettista, le quattro ancore dei numeri, il cash flow come catena completa dal vincolo alla curva, "l'avete fatto davvero?", onestà selettiva), **§9** (alternative scartate in tabella + strumenti del corso non prodotti + alternative di scope negoziate) e **§10** (cosa faremmo di diverso su tre piani: progetto narrato, gestione, elaborato come documento). Corretto lì il ripasso sulle stime: le slide dichiarano efficienza **75%** e interruzioni non pianificate al **33%** del clock time (il documento riportava 25%). Documenti di consegna non toccati. Aggiornamento precedente: 2026-09-06 — **§6.5 (capacità vs carico) con i conti espliciti**: `441 ÷ 12` e `310 ÷ 11` riportati come operazioni rifacibili a voce, con la ragione dei denominatori diversi (Sprint 0 di setup: lavoro sì, user story no) e lo Sprint 5 natalizio quantificato (21/40 SP, 19 di carryover). Le versioni distese, con tabella capacità/carico/velocity e **"Risposta da 30 secondi"**, sono in FAQ C8 (story point) e FAQ B7 (BMC e break-even a 84 mesi); domande-lampo allineate nel DRILL. Documenti di consegna non toccati. Aggiornamento precedente: 2026-09-03 — **Documenti del corso non prodotti, risposte pronte**: **H19** sul **PDS** (attenzione: è citato per nome nelle linee guida dell'elaborato, quindi la domanda è plausibile — risposta su opzionalità dichiarata nelle slide, funzione del PDS come ponte fra senior management e team che qui non serviva perché il POS l'ha scritto il team, cinque funzioni mappate sui documenti prodotti, onboarding mai richiesto in 7 mesi di team invariato) con rimando da H8 e una riga nel DRILL §2. Stesso giorno — **POS vs Project Charter** (FAQ H18 + 3 domande-lampo nel DRILL §2): il Charter è nelle slide, quindi la domanda è attesa — risposta su coerenza con Wysocki, sulla funzione del Charter (conferire autorità sulle risorse) che è vuota dove il PM è già CEO e PO, e sulla mappatura di tutti i contenuti del Charter sui documenti effettivamente prodotti; più la distinzione gate/documento per "chi ha autorizzato il progetto". Stesso giorno — **Business value del progetto messo a fuoco** (FAQ A6 + 3 domande-lampo nel DRILL §2 e una trappola in §7): per Wysocki è il valore percepito dal *destinatario* — la community che rigioca insieme, quantificata nei criteri del POS e interamente su **Improved Service** dell'IRACIS (break-even ≈84 mesi → non-profit dichiarato); il valore per lo spin-off (pilota, modello replicabile, budget ridotto) è la sezione **Opportunità** del POS e non va confuso col business value. Concretizzazione a due tempi: audit del 13/05 su obiettivi e vincoli, valore a regime rimandato al monitoraggio dei primi 3 mesi (anticipatori: 50 registrati e 23 partite in 24 ore). Stesso giorno — **Metodo di costruzione della RBS coperto lato orale**: la relazione (§2.7) lo riassume in una frase sulle fonti e l'Allegato 2.7 non lo dichiara, quindi la risposta col lessico del corso vive nei materiali di studio — nuova **FAQ B14** (5 tecniche su 7 usate, 2 escluse con motivazione, completezza valutata per sottosistema come criterio di scelta del PMLC, evidenze rintracciabili nell'Allegato 2.6 e nelle CoS), rimando da H14, due domande-lampo nel DRILL §2 e un bullet in SCELTE-DI-PROGETTO §2.8. I documenti consegnati restano invariati. Aggiornamento precedente: 2026-08-29 — **Potenziamento del materiale di studio per l'orale**: (1) le 28 domande reali del professore in `../studio/domande_del_professore.md` hanno ora ciascuna un "Aggancio a MaraffaOnline" con fatti e numeri del progetto; (2) nuova sezione **FAQ H** (17 domande) sui concetti del corso che l'elaborato non nomina — Scope Triangle, i quattro creep, classificazione del progetto, Brooks, JPPS/PDS, Scope Bank, Kolb/war room, milestone trend e burn chart, KPI Kanban, knowledge area PMBOK, tecniche di raccolta requisiti, DevOps, resource leveling, PM=CEO=PO — con risposte oneste ("come si mappa" o "perché non serviva"); le definizioni-lampo sono diventate la sezione I; (3) `../studio/guida_elaborato.md` riallineata alla struttura consegnata (numerazione 2.1–2.9, verbali ritirati, 4.2/4.3 separati, critical path 141, accettazione 11/05); (4) nuovo `DRILL-ORALE.md` con ≈100 domande-lampo per il ripasso attivo. Aggiornamento precedente: 2026-08-24 — **Sezioni 4.4 Comunicazione e 4.5 Gestione del Cambiamento della relazione arricchite e rese autonome** (stile relazione di riferimento): tolti i due rimandi all'Allegato 4.3, il cui contenuto di sintesi (ex sezioni 6-7) vive ora per intero nella prosa della relazione; l'Allegato 4.3 v.1.1.0 torna alle sole regole operative (problem solving, decision making, conflict resolution, brainstorming, meeting). Nel Capitolo6 §6.1 l'installazione è ora classificata come **approccio a fasi** (beta 20 tester → go-live pubblico; cut-over/parallel non applicabili senza legacy), rispondendo alla teoria del modulo Closing. Il documento REGOLE-UFFICIALI-MARAFFONE (già nel pacchetto ma orfano) è ora linkato dalla §2.10 della relazione, senza numero di allegato. Conteggi: **28 URI uniche** (i tre link puntavano allo stesso file: ora RegoleOperative è linkato una volta sola, dalla §4.3) e 28 PDF in Documentazione. Aggiornamento precedente: 2026-08-23 — **Asciugatura finale dello Scoping** in tre mosse, calibrata sulla relazione di riferimento: (1) **verbali ritirati dagli allegati** — Project Scoping Meeting (ex 2.1) e Approval Process (ex 2.11) sono ora solo narrati nella relazione (§2.1 e §2.11); i file restano nel repo come `Scoping/_ritirato-*.md`, nomi fuori dal pattern `Allegato*` così il generatore li ignora da solo; (2) **allegati Scoping rinumerati sequenzialmente 2.1–2.9** (CoS=2.1, POS=2.2, Risk=2.3, BMC=2.4, SWOT=2.5, Prototyping=2.6, RBS=2.7, US=2.8, PMLC=2.9) con riferimenti aggiornati in tutto il progetto, versioni bumpate e changelog; il numero di allegato NON coincide più con la sezione della relazione (risposta pronta in FAQ G15); (3) **RBS asciugata al livello di requisito** (rimossi i dettagli operativi, che vivono in US/regole ufficiali/CoS/Planning; corretto il tag dell'IA da Could a Won't). Conteggi attesi: **27 URI** e **28 PDF** in Documentazione. In serata: **audit teorico contro la knowledge_base** (6 revisori paralleli, esito in `SCELTE-DI-PROGETTO.md` §8 terzo giro) — corretti Should Have ("critici, sostituibili se necessario"), legenda SS + convenzione dei tempi CPM dichiarata (FAQ G16), doppia lettura WBS ("task" del corso = le 43 attività), rischio 1.2 Avoid e 4.2 Mitigazione, tabella PMLC del 2.9 riallineata al quadrante (via "Scrum-like" e "Kanban" come etichette di famiglia), sei righe RASCI A→A/R, Project Notebook + FPR sul template del corso, business value a regime rimandato al post-lancio, colonna Net Cash Flow rinominata, "US (2.9)"→"US (2.8)" nel Backlog; EVM, stoplight, escalation e passi del Closing verificati senza rilievi. Aggiornamento precedente: 2026-08-11 (sera) — **Capitoli 5 e 6 snelliti a registri operativi**: Cap. 6 Closing da 7 a 4 pagine (audit e lessons learned in tabella), Cap. 5 Monitoring da 9 a ≈3 pagine in due passate, calibrate sul riferimento relazioneSofy (tolte le percentuali decorative e le colonne CV/SV; velocity in prosa; restano tabella EVM mensile e Stoplight di gennaio, richiesti dalle linee guida). Narrazione tolta perché già nella relazione; tutti i fatti e i numeri invariati, numerazione sezioni stabile (5.1-5.11, 6.1-6.8). In giornata: **revisione di granularità completata e ri-verificata**: WBS v.1.3.0 con doppia lettura dichiarata (43 attività di controllo + 160 task operativi), RASCI v.2.1.0 potata a 51 righe a livello attività (dimostrato: stessi 48 pattern di ruoli della versione a 124 righe; md e HTML coincidono cella per cella), tabella di raccordo WBS↔Network nell'Allegato 3.5 v.2.1.0 (tutte le 43 attività coperte); nuova risposta pronta §6.10 e FAQ C13; sweep totale di coerenza eseguito (riferimenti, versioni, conteggi), PDF e consegna rigenerati. Aggiornamento precedente: 2026-08-10 — **Allegato 4.2 separato in due**: 4.2 RASCI Matrix (matrice + companion HTML visuale) e 4.3 Regole Operative, queste ultime riscritte in forma schematica (tabelle per regola, 4 pagine invece delle 14 dell'allegato unico) e arricchite col Backlog Refinement fra le cerimonie; il Cap. 4 della Relazione linka i tre allegati e PDF/ZIP di consegna sono stati rigenerati. Aggiornamento precedente: 2026-08-09 (sera) — audit incrociato a 5 lenti completato e correzioni applicate a tutti gli allegati (vedi §7.6); PDF e Relazione rigenerati; creati i documenti gemelli `SCELTE-DI-PROGETTO.md` (registro ragionato delle scelte) e `FAQ.md` (domande previste all'orale con risposte). Aggiornamento precedente: 2026-08-02 — allineato ad Allegato 3.3 v.3.1.0 (US reali del 2.9, sprint da Gantt), Allegato 3.5 v.2.0.0 (critical path 141 gg, fast tracking P→R), RASCI v.1.4.0, riconciliazione capacità 40 / carico 37-28, ritardo di gennaio su Dashboard (M), audit di coerenza incrociata completato.
