...
\section{Sprint Review e Retrospective}
...
\section{Reporting: Stoplight Reports}
...
\begin{itemize}
    \item \textbf{Scope: Verde} - Tutte le feature Must Have sono in sviluppo secondo il piano. Nessuna change request ricevuta da Giovanni.
    \item \textbf{Schedule: Giallo} - Il Frontend Tavolo da Gioco, attività sul critical path, ha accumulato 3 giorni di ritardo a causa della complessità inaspettata nel drag \& drop delle carte. Recovery plan attivo: Luca Moretti è affiancato da Sara Bianchi per pair programming. Previsto recupero entro fine Sprint 9.
    \item \textbf{Budget: Verde} - Spesi €3.500 su €3.700 previsti per il Mese 1 (Novembre). Sotto budget del 5\%. Cash flow positivo con saldo cumulativo di €5.800.
    \item \textbf{Quality: Verde} - Test coverage all'87\%, superiore al target del 85\%. Zero bug critici aperti. Tutti i test di integrazione superati.
    \item \textbf{Risks: Giallo} - Rischio latenza WebSocket ancora in monitoring. Load test previsto per Sprint 6 mostrerà se la mitigazione (ottimizzazione Socket.IO) è sufficiente.
\end{itemize}
...
Per il progetto MaraffaOnline, Marco Venturi ha creato un foglio di calcolo Excel condiviso con Giovanni (in modalità read-only per lo sponsor) dove vengono tracciati mensilmente i valori di PV, EV e AC. Questi dati vengono poi visualizzati in un grafico che mostra l'andamento delle tre curve nel tempo, e sono sintetizzati nella tabella EVM riportata più avanti. Il grafico permette di identificare rapidamente se il progetto è on schedule (EV allineato con PV), under budget (AC inferiore a EV), o se ci sono problemi di performance (EV inferiore a PV indica che si sta producendo meno valore di quanto pianificato).

Considerando il periodo Ottobre 2025 - Gennaio 2026, corrispondente ai primi quattro mesi del progetto, si osserva una buona aderenza tra le tre curve. A fine Gennaio 2026, il progetto mostrava i seguenti valori:

\begin{itemize}
    \item \textbf{Planned Value (PV)}: €14.100 - Questo è il budget che era pianificato per i primi 4 mesi secondo il Cash Flow Management (€3.000 per il Mese 0 + €3.700 $\times$ 3 mesi per Novembre, Dicembre e Gennaio).
    \item \textbf{Earned Value (EV)}: €13.200 - Valore del lavoro effettivamente completato, leggermente inferiore al pianificato.
    \item \textbf{Actual Cost (AC)}: €13.500 - Costi reali sostenuti, leggermente superiori al valore guadagnato.
\end{itemize}

Da questi valori possiamo calcolare due indici fondamentali:

\textbf{Cost Variance (CV) = EV - AC = €13.200 - €13.500 = -€300}
...
\textbf{Schedule Variance (SV) = EV - PV = €13.200 - €14.100 = -€900}
...
Si può notare come nel complesso il progetto abbia mantenuto le tre linee relativamente vicine, segno di una buona capacità di pianificazione ed esecuzione. In altri momenti dell'anno, in particolare durante i mesi di Ottobre e Novembre 2025 quando il team lavorava principalmente su sistemi Waterfall e setup infrastrutturale con requisiti ben definiti, si è riusciti per lunghi periodi a mantenere le tre linee molto vicine.

\subsection{Riepilogo EVM mensile (Ott 2025 - Mag 2026)}

Per completezza, la tabella seguente riassume i valori EVM mese per mese fino alla chiusura del progetto:

\begin{center}
\footnotesize
\begin{tabular}{clrrrrrcc}
\toprule
Mese & Periodo & PV (€) & EV (€) & AC (€) & CV (€) & SV (€) & CPI & SPI \\
\midrule
...
I valori di PV corrispondono all'outflow cumulato pianificato nel Cash Flow Management (Allegato 3.4). La traiettoria mostra come il leggero overrun di Gennaio (CV=-300, SV=-900) sia stato progressivamente assorbito attraverso il recovery plan, fino a chiudere a fine progetto con CPI=1,00 e SPI=1,00. Il consumo finale di €22.750 lascia il surplus di €2.250 documentato nel Capitolo 6 - Closing.

\section{Gantt Chart Tracking}
...
\section{Velocity Tracking per Sistemi Agile}
...
\textbf{Nota metodologica sulla velocity tracciata}: i 40 story points/sprint sono la \textbf{capacity team-wide} stimata, che include sia il lavoro sulle user stories del Product Backlog Agile (Allegato 3.3, 310 SP totali = $\sim$21 SP/sprint di pura velocity Scrum) sia il lavoro sui sottosistemi gestiti con altre metodologie (Game Engine in Waterfall, Infrastructure in Incrementale, per $\sim$130 SP equivalenti complessivi). La metrica monitorata in questo capitolo è quindi una velocity \textit{gestionale} a livello di team, non la velocity Scrum pura, per dare a Giovanni Marchetti una visione unica e consolidata dell'avanzamento del lavoro su tutto il progetto.
...
\begin{itemize}
    \item \textbf{Sprint 1} (28 Ott - 08 Nov): 38 story points completati su 40 pianificati (95\%)
    \item \textbf{Sprint 2} (11 Nov - 22 Nov): 42 story points completati su 40 pianificati (105\%)
    \item \textbf{Sprint 3} (25 Nov - 06 Dic): 40 story points completati su 40 pianificati (100\%)
    \item \textbf{Sprint 4} (09 Dic - 20 Dic): 35 story points completati su 40 pianificati (87,5\%) - Impatto festività
    \item \textbf{Sprint 5} (06 Gen - 17 Gen): 39 story points completati su 40 pianificati (97,5\%)
    \item \textbf{Sprint 6} (20 Gen - 31 Gen): 36 story points completati su 40 pianificati (90\%) - Ritardo Frontend
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Test Coverage minimo 85\%}: ogni componente deve avere test unitari che coprono almeno l'85\% del codice. Questo garantisce che le funzionalità siano verificate automaticamente e che eventuali regressioni vengano identificate rapidamente.
    \item \textbf{Zero bug critici aperti}: prima di considerare una feature completata, tutti i bug di severità critica devono essere risolti. I bug di severità media o bassa possono essere posticipati a sprint successivi se non bloccano funzionalità core.
    \item \textbf{Code review approvata}: ogni pull request deve essere rivista e approvata da almeno un altro membro del team prima del merge. Nel caso di modifiche particolarmente critiche (ad esempio, logica del Game Engine o sincronizzazione Real-Time), la code review deve essere effettuata da Elena Rossi, la Tech Lead.
    \item \textbf{Performance requirements soddisfatti}: per il sistema Real-Time Communication, la latenza deve essere inferiore a 500ms. Questo viene verificato attraverso load test automatizzati che simulano 50 partite simultanee.
\end{enumerate}

Durante i Project Status Meeting, Elena Rossi presenta un report sulle quality metrics che include il Test Coverage attuale (monitorato attraverso tool di code coverage integrati nella CI/CD pipeline: durante Gennaio 2026 il coverage era all'87\%, superiore al target dell'85\%), il Bug Burn Down (grafico che mostra il trend dei bug aperti nel tempo, suddivisi per severità), la Code Complexity (metriche come il Cyclomatic Complexity per identificare porzioni di codice che potrebbero beneficiare di refactoring) e il Technical Debt (stima del tempo necessario per ``pulire'' il codice; PlayHeritage Labs ha stabilito che non deve superare il 10\% del tempo totale di sviluppo).
...
Marco Venturi mantiene un Risk Log su Confluence che traccia lo stato di ciascun rischio identificato nella Risk Rating Matrix. Per ogni rischio vengono monitorati lo status attuale (Aperto, In Mitigation, Mitigato, Materializzato), la probabilità e l'impatto aggiornati sulla base delle informazioni emerse durante il progetto, le azioni di mitigazione intraprese e l'owner responsabile di monitorarlo e attivare il piano di contingenza.
...
\section{Risultati del Monitoraggio}
...
\textbf{Criteri Temporali}: il progetto ha accumulato un massimo di 3 giorni di ritardo sul critical path durante Gennaio 2026, ma grazie al recovery plan il ritardo è stato recuperato entro fine Febbraio. La data di lancio del 15 Maggio 2026 è stata mantenuta.

\textbf{Criteri Economici}: il budget di €25.000 è stato rispettato. A fine Febbraio 2026 (Mese 4) le spese totali erano di €17.500 su €17.800 pianificate, con uno scostamento favorevole di €300 (1,7\%), ben all'interno del Contingency Buffer del 18,7\%.

\textbf{Criteri Tecnici}: tutte le regole del Maraffone/Beccaccino sono state implementate correttamente e validate da Francesca Giuliani durante la Sessione 1 di validazione (Novembre 2025). Il sistema Real-Time mantiene una latenza media di 185ms, sotto il target di 500ms. Il test coverage è stabilmente sopra l'85\%.

\textbf{Criteri Qualitativi}: l'esperienza utente è stata validata in due momenti. Una prima sessione a Gennaio 2026 con 5 membri della community Maraffa Forever ha dato riscontri positivi sull'usabilità e portato a piccole migliorie implementate nello Sprint 11. La User Acceptance Testing ufficiale, condotta con 10 tester della community nella prima metà di Maggio 2026, ha poi confermato che l'80\% degli utenti è riuscito a completare una partita senza aiuto, soddisfacendo il criterio di usabilità definito nelle Conditions of Satisfaction.

\textbf{Criteri di Gestione del Lavoro}: il team ha lavorato in modo coeso e collaborativo. Le Sprint Retrospective hanno prodotto 30 action items di miglioramento continuo nel corso dei 15 sprint, di cui 28 sono stati completamente implementati, portando a un progressivo aumento dell'efficienza del team.
